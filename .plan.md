...
5. [x] Replace fixed recording with Silero VAD end-of-utterance
   - Add `voice/src/vad.py` wrapper with `is_speech(frame)` and `stream_until_silence`
   - Update recording flow to stop after `min_silence_duration` once speech starts
   - Configurable thresholds in `config.yaml`
   - Tests:
     - `voice/tests/unit/test_vad.py` (pre-recorded WAVs, stop timing)
     - `voice/tests/functional/test_vad.py` (live mic, optional)
6. [ ] Replace `say` with AVSpeechSynthesizer (for cancellation support) (in progress)
   - Use macOS AVSpeechSynthesizer; keep `say` as fallback
   - Tests:
     - `voice/tests/unit/test_tts.py` (stub cancel)
     - `voice/tests/functional/test_tts.py` (speaker, optional)
7. [x] Add VoiceState enum, refactor into state machine
   - Refactor slice logic into orchestrator callbacks
   - Tests:
     - `voice/tests/unit/test_state.py` (valid transitions, barge-in behavior)
     - Unit tests for event → transition mapping
8. [x] Add Unix socket server for UI communication
   - Add `voice/src/ui_socket.py` JSON-lines server at `/tmp/openclaw_voice.sock`
   - Broadcast `state_change`, `transcription`, `response_chunk`, `waveform` events
   - Tests:
     - `voice/tests/unit/test_ui_socket.py` (event round-trip)
     - `voice/tests/functional/test_ui_socket.py` (reconnect, optional)
9. [ ] Build minimal SwiftUI overlay (top-right, text + ring animation)
   - Create `ui/JarvisOverlay/` app with `NSPanel` always-on-top
   - Ring animation driven by state; show transcription text
   - Tests:
     - Manual run with `--test-cycle`
     - Minimal XCTest for CLI arg parsing
10. [x] Wire UI to voice engine via socket
   - Implement socket client in Swift (`VoiceEngine.swift`) reading JSON lines
   - Emit state changes from Python engine
   - Tests:
     - `voice/tests/unit/test_ui_socket.py` verifies events emitted
     - Manual end-to-end run (engine + UI)
